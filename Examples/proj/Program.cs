...
class InferResourceLeakTests
{
    ...
    /// <summary>
    /// This was a former false positive that would occur because async methods with a using
    /// statement would only execute the disposal finally block if the __state field generated by
    /// the compiler was negative; Infer did not know that a separate initializer method would 
    /// initialize this field to a negative value. 
    /// </summary>
    public async Task<object> AsyncUsingShouldNotReport()
    {
        var y = new object();
