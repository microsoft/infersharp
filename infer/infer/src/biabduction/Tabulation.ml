...
let get_specs_from_payload summary =
  (* Summary.pp_text Format.std_formatter summary ; *)
...
and sexp_star_fld se1 se2 : Sil.strexp =
  ...
  | _ ->
      L.d_str "sexp_star_fld cannot star " ;
      Sil.d_sexp se1 ;
      L.d_str " and " ;
      Sil.d_sexp se2 ;
      L.d_ln () ;
...
let sigma_star_fld tenv (sigma1 : Sil.hpred list) (sigma2 : Sil.hpred list) : Sil.hpred list =
  ...
  with exn when SymOp.exn_not_failure exn ->
    L.d_str "sigma_star_fld cannot star " ;
    L.d_ln () ;
    Prop.d_sigma sigma1 ;
    L.d_ln () ;
    L.d_str " and " ;
...
let sigma_star_typ (sigma1 : Sil.hpred list) (typings2 : (Exp.t * Exp.t) list) : Sil.hpred list =
  ...
  with exn when SymOp.exn_not_failure exn ->
    L.d_str "sigma_star_typ cannot star " ;
    Prop.d_sigma sigma1 ;
    L.d_str " and " ;
...
let exe_spec exe_env tenv ret_id (n, nspecs) caller_pdesc callee_pname loc prop path_pre
  ...
  | Prover.ImplOK
      ...
      (* print_endline "====================================================================" ;
      print_endline "Split: ";
      List.iter ~f:(fun hpred -> Sil.pp_hpred Pp.text Format.std_formatter hpred; print_endline "") frame ;
      print_endline "====================================================================" ; *)
...
let print_abduction_res r =
  match r with
  | Valid_res vr ->
    Printf.printf "Valid_res - IPM: %B" vr.incons_pre_missing
  | Invalid_res ir ->
    Printf.printf "Invalid_res" 
