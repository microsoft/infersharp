...
module Worklist = struct
  ...
  let pp_todo_set (wl: t): unit =
    NodeVisitSet.iter (fun vn -> Printf.printf "%d " (Procdesc.Node.get_id vn.node :> int)) wl.todo_set
end
...
let propagate (wl : Worklist.t) pname ~is_exception (pset : Paths.PathSet.t)
  ...
  (* Printf.printf "Node: %d Changed: %B\n" (Procdesc.Node.get_id curr_node :> int) (changed) ; *)
...
let propagate_nodes_divergence tenv (proc_cfg : ProcCfg.Exceptional.t) (pset : Paths.PathSet.t)
  ...
  (*Printf.printf "PND %d\n" (Procdesc.Node.get_id curr_node :> int);*)
  ...
  Printf.printf "methodname: %s" pname_string ;
  print_endline "";*)
...
let forward_tabulate summary exe_env tenv proc_cfg wl =
  ...
  let do_node_and_handle curr_node session =
    ...
    try
      ...
      in
     (* Printf.printf "FT wl todo: " ;
      Worklist.pp_todo_set wl ;
      print_endline "";*)
    ...
    let get_results (wl : Worklist.t) () =
      ...
      let specs =
        ...
        with Exceptions.Leak _ ->
          ...
          in
          Reporting.log_issue_deprecated_using_state Exceptions.Error pname exn ;
          print_endline "returning no specs" ;
          (* retuning no specs *) []
      in
...
let report_runtime_exceptions tenv pdesc summary =
  ...
  let is_main =
    ...
    | Typ.Procname.CSharp pname_csharp ->
        Typ.Procname.CSharp.is_static pname_csharp
        && String.equal (Typ.Procname.CSharp.get_method pname_csharp) "Main"
    | _ ->
        false
  in
...
let perform_transition proc_cfg tenv proc_name summary =
  ...
  (*  Printf.printf "perform_transition %s\n" (Typ.Procname.get_method proc_name) ;*)
...
let analyze_procedure {Callbacks.summary; proc_desc; tenv; exe_env} : Summary.t =
  ...
  (*Printf.printf "Interproc proc %s\n" (Typ.Procname.get_method (Procdesc.get_proc_name proc_desc)) ;*)
  BuiltinDefn.init () ;
