...
let resolve_virtual_pname tenv prop actuals callee_pname call_flags : Typ.Procname.t list =
  ...
  | (receiver_exp, actual_receiver_typ) :: _ ->
      if !Language.curr_language <> Language.Java && !Language.curr_language <> Language.CIL then
    ...
    if not (is_rec_call callee_pname pdesc) then
      ...
      let prop_with_abduced_var =
        let abduced_ret_pv =
          ...
          let loc = if (Typ.Procname.is_java callee_pname || Typ.Procname.is_csharp callee_pname) then Location.dummy else callee_loc in
...
let rec sym_exec exe_env tenv current_pdesc instr_ (prop_ : Prop.normal Prop.t) path :
 ...
 (* Printf.printf "Symexec: %s | " (Typ.Procname.get_method current_pname);
  Sil.pp_instr ~print_types:true Pp.text F.std_formatter instr_ ;
  print_endline "" ; *)
      ...
      let check_condition_always_true_false () =
        ...
        then
          ...
          | _ ->
              ()
      in
      if not (Config.tracing || Typ.Procname.is_java current_pname || Typ.Procname.is_csharp current_pname) then
    ...
    | None -> (
      ...
      | CSharp callee_pname_csharp ->
          ...
          let exec_one_pname pname =
            ...
            match Ondemand.analyze_proc_name ~caller_pdesc:current_pdesc pname with
            | None ->
                let ret_typ = Typ.Procname.CSharp.get_return_typ callee_pname_csharp in
                let ret_annots = load_ret_annots callee_pname in
                exec_skip_call ~reason:"unknown method" ret_annots ret_typ
            | Some callee_summary -> (
              match reason_to_skip ~callee_desc:(`Summary callee_summary) with
...
and unknown_or_scan_call ~is_scan ~reason ret_typ ret_annots
  ...
  let pre_final =
    (* in Java or C#, assume that skip functions close resources passed as params *)
    let pre_1 = if (Typ.Procname.is_java callee_pname || Typ.Procname.is_csharp callee_pname) then remove_file_attribute pre else pre in
    let pre_2 =
