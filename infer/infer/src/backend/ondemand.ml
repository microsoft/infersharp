...
let analyze_proc ?caller_pdesc callee_pdesc =
  let caller_pname = match caller_pdesc with | Some pd -> Typ.Procname.get_method (Procdesc.get_proc_name pd) | None -> "" in
    ...
    with Caml.Not_found ->
      let summary_option, update_memcached =
        ...
        | Some summ_opt ->
            (summ_opt, false)
        | None ->
            let sa = procedure_should_be_analyzed callee_pname in
