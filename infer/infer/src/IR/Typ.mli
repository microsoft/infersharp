...
module Procname : sig
  ...
  module CSharp : sig
    ...
    type csharp_type = Name.CSharp.Split.t

    val constructor_method_name : string

    val make : string -> csharp_type list -> Name.t -> csharp_type option -> kind -> t
    (** Create a CSharp procedure name from its
...
being the name of the struct, [None] means the parameter is of some other type. *)
  ...
  (** Check if this is a CSharp procedure naem. *)
...
module Fieldname : sig
  ...
  module CSharp: sig
    val from_string : string -> t
    (** Create a csharp field name from string *)
  end
