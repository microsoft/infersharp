...
let print_cfg_procs cfg =
  let f pname pdesc = 
    match pname with
    | Typ.Procname.CSharp pn ->
      print_endline (Typ.Procname.CSharp.get_method pn) ;
      ProcAttributes.pp Format.std_formatter (Procdesc.get_attributes pdesc) ;
      print_endline "" ;
      Procdesc.print_pdesc_nodes pdesc ;
      print_endline "" 
    | _ ->
      ()
  in
