...
module Name = struct
  ...
  module CSharp = struct
    ...
    let from_string name_str = CSharpClass (Mangled.from_string name_str)
...
module Procname = struct
  ...
  module CSharp = struct
    ...
    type csharp_type = Name.CSharp.Split.t = {namespace: string option; type_name: string}
    [@@deriving compare]

    let csharp_void = {namespace= None; type_name= "void"}

    type t =
      { method_name: string 
      ; parameters: csharp_type list
      ; class_name: Name.t
      ; return_type: csharp_type option (* option because constructors have no return type*)
      ; kind: kind }
    [@@deriving compare]

    let make method_name parameters class_name return_type kind =
      {method_name; parameters; class_name; return_type; kind} 
    ...
    let type_to_string_verbosity p verbosity =
      match p with
      | {namespace= Some namespace; type_name} when is_verbose verbosity ->
          namespace ^ "." ^ type_name
      | {type_name} ->
          type_name

    let csharp_type_of_name class_name = Name.CSharp.Split.of_string (Name.name class_name)
    ...
    let return_type_to_string cs verbosity =
      match cs.return_type with None -> "" | Some typ -> type_to_string_verbosity typ verbosity

    let get_class_name cs = Name.name cs.class_name

    let get_class_type_name cs = cs.class_name

    let get_return_type cs = return_type_to_string cs Verbose

    let get_method cs = cs.method_name

    let get_return_typ pname_csharp =
      let rec csharp_from_string = function
        | "" | "void" ->
            mk Tvoid
        | "int" ->
            mk (Tint IInt)
        ...
        | typ_str when String.contains typ_str '[' ->
            ...
            mk (Tptr (mk_array (csharp_from_string stripped_typ), Pk_pointer))
        | typ_str ->
            mk (Tstruct (Name.CSharp.from_string typ_str))
      in
      let typ = csharp_from_string (get_return_type pname_csharp) in
    ...
    let to_string ?(withclass = false) cs verbosity =
      ...
      | Verbose | Non_verbose ->
         ...
         verbose is used for example to create unique filenames, non_verbose to create reports *)
          let return_type = return_type_to_string cs verbosity in
          let params = param_list_to_string cs.parameters verbosity in
          let class_name =
            type_to_string_verbosity (Name.CSharp.split_typename cs.class_name) verbosity
          in
          let separator =
            match (cs.return_type, verbosity) with
            | None, _ ->
                ""
            | Some _, Verbose ->
                ":"
            | _ ->
                " "
          in
          let output = class_name ^ "." ^ cs.method_name ^ "(" ^ params ^ ")" in
      ...
      | Simple ->
          ...
          let method_name =
            if String.equal cs.method_name constructor_method_name 
            then 
              (* cil todo *)
              (* get_simple_class_name cs *)
              cs.method_name
            else cls_prefix ^ cs.method_name
          in
          method_name ^ "(" ^ params ^ ")"
  end
  ...
  being the name of the struct, [None] means the parameter is of some other type. *)
    ...
    type t = JavaParameter of Java.java_type | ClangParameter of clang_parameter | CSharpParameter of CSharp.csharp_type
    [@@deriving compare]
  ...
  let rec to_string p =
    ...
    | CSharp cs ->
        CSharp.to_string cs Non_verbose
    | C osig ->
  ...
  let rec to_simplified_string ?(withclass = false) p =
    ...
    | CSharp cs ->
        CSharp.to_string ~withclass cs Simple
    | C osig ->
  ...
  let hashable_name p =
    ...
    | CSharp pname ->
        Str.global_replace (Str.regexp "\\$[0-9]+") "$_"
          (CSharp.to_string ~withclass:true pname Simple)
  ...
  let parameter_of_name procname class_name =
    ...
    | CSharp _ ->
        Parameter.CSharpParameter (CSharp.csharp_type_of_name class_name)
    | _ ->
...
module Fieldname = struct
  type t = Clang of {class_name: Name.t; field_name: string} | Java of string | CSharp of string
  [@@deriving compare]
  ...
  let to_string = function Java fname -> fname | Clang {field_name} -> field_name | CSharp fname -> fname
  ...
  let pp f = function Java field_name | CSharp field_name | Clang {field_name} -> Format.pp_print_string f field_name
